...
const { sendJson, supabaseAdminRequest } = require('./_lib');
...
module.exports = async function handler(req, res) {
  ...
  res.setHeader('Cache-Control', resource === 'plans' ? CACHE_PLANS : CACHE_AGGREGATE);

  try {
    if (!resource) {
      const [content, tracks, plans] = await Promise.all([
        fetchContent(), fetchTracks(), fetchPlans()
      ]);
    ...
    if (resource === 'content') {
      ...
      return respond(res, method, { ok: true, data: r.data, version: r.version, updatedAt: r.updatedAt });
    }
    ...
    if (resource === 'plans') {
      ...
      return respond(res, method, { ok: true, currency: r.currency, plans: r.plans });
    }
...
async function fetchTracks() {
  const result = await supabaseAdminRequest(
    `/rest/v1/tracks?published=eq.true&select=album_id,track_index,title,duration,preview_start,preview_duration,price_cents,for_sale,full_audio,preview_audio,lyrics&order=album_id.asc,track_index.asc&limit=${MAX_TRACKS}`,
    { method: 'GET' }
  );
  if (!result.response.ok) return { ok: false };
  ...
  return {
    ok: true,
    tracks: rows.map((t) => ({
      albumId: t.album_id, trackIndex: t.track_index,
...
async function fetchPlans() {
  ...
  const plans = [{ id: 'free', priceCents: 0, interval: null, available: true }];
...
function fail(res, method, status, message) {
  res.setHeader('Cache-Control', 'no-store');
  if (method === 'HEAD') { res.status(status); return res.end(); }
