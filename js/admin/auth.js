...
export function showPasswordChangeForm() {
  ...
  const newPass =
    document.getElementById('adminNewPass');
...
export async function openAdminSite() {
  ...
  try {
    ...
    if (err?.status === 403) {
      setError(
        'adminLoginError',
        'Acesso rejeitado pelo servidor.'
      );
...
function bindLoginElements() {
  ...
  if (forgotBtn) {
    forgotBtn.addEventListener('click', () => {
      setError(
        'adminLoginError',
        'Para redefinir a senha, gere um novo hash com o script hash-admin-password.js e atualize ADMIN_PASSWORD_HASH no servidor.'
      );
    });
  }
